...
class WellnessForeverScraper(BaseScraper):
    platform_slug = "wellness_forever"
    platform_name = "Wellness Forever"
    base_url = "https://www.wellnessforever.com"
                ...
                try:
                    ...
                    if price is not None:
                        results.append(ScrapedPrice(
                            ...
                            price=price, mrp=price, discount_pct=None, is_available=True,
                            product_url=product_url, product_name_raw=None,
                        ))
                        break
                except Exception:
                    continue
        except Exception as e:
            self.log.warning("WellnessForever scrape failed", name=name, error=str(e))
        finally:
            await page.close()
        if not results:
