...
class MedPlusScraper(BaseScraper):
    platform_slug = "medplus"
    platform_name = "MedPlus Mart"
    base_url = "https://www.medplusmart.com"

    def _format_search_url(self, query: str) -> str:
        return f"{self.base_url}/search#searchTxt={urllib.parse.quote(query)}"
    ...
    async def _scrape_medication(self, context, medication: dict) -> list[ScrapedPrice]:
        ...
        try:
            ...
            cards = await page.query_selector_all('.product-box, [class*="product"]')
            for card in cards[:2]:
                try:
            ...
            self.log.warning("MedPlus scrape failed", name=name, error=str(e))
        finally:
            await page.close()
        if not results:
            results.append(ScrapedPrice(platform_slug=self.platform_slug, medication_name=name, strength=strength,
